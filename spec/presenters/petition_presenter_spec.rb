...
RSpec.describe PetitionPresenter do
  ...
  describe '#child_arrangements_orders' do
    it 'only returns the orders set to true' do
      expect(subject.child_arrangements_orders).to eq([:child_home])
    end
  end

  describe '#specific_issues_orders' do
    it 'only returns the orders set to true' do
      expect(subject.specific_issues_orders).to eq([:child_specific_issue_religion, :child_specific_issue_medical])
    end
  end

  describe '#prohibited_steps_orders' do
    it 'only returns the orders set to true' do
      expect(subject.prohibited_steps_orders).to eq([:child_flight])
    end
  end

  describe '#all_selected_orders' do
    it 'only returns the orders set to true' do
      expect(subject.all_selected_orders).to eq([:child_home, :child_specific_issue_religion, :child_specific_issue_medical, :child_flight])
    end
  end
end
