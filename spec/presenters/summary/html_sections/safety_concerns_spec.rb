  ...
  describe HtmlSections::SafetyConcerns do
    ...
    describe '#answers' do
      it 'has the correct rows' do
        ...
        expect(answers[2].question).to eq(:substance_abuse)
        ...
        expect(answers[3].question).to eq(:substance_abuse_details)
        expect(answers[3].value).to eq('substance details')
        expect(answers[3].change_path).to eq('/steps/safety_questions/substance_abuse_details')

        expect(answers[4]).to be_an_instance_of(Answer)
        expect(answers[4].question).to eq(:children_abuse)
        expect(answers[4].change_path).to eq('/steps/safety_questions/children_abuse')
        expect(answers[4].value).to eq('no')

        expect(answers[5]).to be_an_instance_of(Answer)
        expect(answers[5].question).to eq(:domestic_abuse)
        expect(answers[5].change_path).to eq('/steps/safety_questions/domestic_abuse')
        expect(answers[5].value).to eq('no')

        expect(answers[6]).to be_an_instance_of(Answer)
        expect(answers[6].question).to eq(:other_abuse)
        expect(answers[6].change_path).to eq('/steps/safety_questions/other_abuse')
